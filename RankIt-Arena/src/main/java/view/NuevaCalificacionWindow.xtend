...
class NuevaCalificacionWindow extends Dialog<ModeloAdminCalificaciones>{

		new(WindowOwner owner, ModeloAdminCalificaciones model) {
		super(owner, model)
	}
	

	override protected createFormPanel(Panel mainPanel) {
		title = "Crear nueva Calificacion"
		new Label(mainPanel).text = "Fecha de registro"
			
			
			crearLabelConTextbox(mainPanel, "Fecha", "nuevaCalificacion.fechaCalificacion")
			crearLabelConTextbox(mainPanel, "Evaluado", "nuevaCalificacion.evaluado")
			crearLabelConTextbox(mainPanel, "Ptos", "nuevaCalificacion.puntos")
